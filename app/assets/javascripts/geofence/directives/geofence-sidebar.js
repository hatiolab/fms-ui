angular.module('fmsMonitor').directive('geofenceSidebar', function() {
	return {
		restrict: 'E',
		controller: 'geofenceSidebarCtrl',
		templateUrl: '/assets/geofence/views/geofence-sidebar.html',
		scope: {},
		link: function(scope, element, attr, geofenceSidebarCtrl) {
			// 버튼이 Directive Element 바깥쪽에 있어서 버튼 클릭함수를 이용 ...
			var refreshButton = angular.element('#searchGeofences');
			refreshButton.bind("click", function() {
				var geofenceTab = angular.element('#geofenceTab');
				// side-fleets 탭이 액티브 된 경우만 호출하도록 변경 ...
				if (geofenceTab.hasClass('active')) {
...
.controller('geofenceSidebarCtrl', function($rootScope, $scope, $resource, $element, GridUtils, FmsUtils, RestApi) {
