angular.module('fmsMonitor').directive('relationSidebar', function() {
	return {
		restrict: 'E',
		controller: 'relationSidebarCtrl',
		templateUrl: '/assets/geofence/views/relation-sidebar.html',
		scope: {},
		link: function(scope, element, attr, relationSidebarCtrl) {
			// 버튼이 Directive Element 바깥쪽에 있어서 버튼 클릭함수를 이용 ...
			var refreshButton = angular.element('.panel-refresh');
			refreshButton.bind("click", function() {
				var relationTab = angular.element('#relationTab');
				// side-fleets 탭이 액티브 된 경우만 호출하도록 변경 ...
				if (relationTab.hasClass('active')) {
...
.controller('relationSidebarCtrl', function($rootScope, $scope, $resource, $element, GridUtils, FmsUtils, RestApi) {
